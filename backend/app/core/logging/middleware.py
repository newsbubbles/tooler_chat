    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if request_body and content_type and "application/json" in content_type:
            try:
                ...
                parsed_body = sanitize_data(parsed_body)
            except Exception:
                parsed_body = {"_note": "Could not parse JSON body", "_raw": request_body[:MAX_CONTENT_LENGTH]}
        elif request_body:
        ...
        try:
            ...
            if response_body and resp_content_type and "application/json" in resp_content_type:
                try:
                    ...
                    # Redact sensitive fields
                    parsed_response = sanitize_data(parsed_response)
                except Exception:
                    # Non-parseable JSON
